...
MMI_EXTERN boolean_T gblExtModeEnabled;
#include "ParamTuningTgtAppSvc/ParamTuningTgtAppSvcCIntrf.h"
   ...
   EXTERN_C const char *TgtConnInit(int_T argc, char_T *argv[])
   {
    const char *result = NULL; /* assume success */
       if (!gblExtModeEnabled) return(result);{   int_T tvar;   for (tvar = 1; tvar < argc; tvar++) {      if (argv[tvar] == NULL) continue;      if (strcmp(argv[tvar],"-tgtconn_port") == 0) {         char       tmpstr[2];         const char *portStr = (char *)argv[tvar+1];         int itemsConverted = sscanf(portStr, "%hd%1s", &port, tmpstr);         if (itemsConverted != 1 || ((port != 0 && port < 255) || port > 65535)) {            result = "Illegal port value for target connectivity comm service\n";            return(result);         }         argv[tvar++] = NULL;         argv[tvar]   = NULL;         continue;      }      if (strcmp(argv[tvar],"-tgtconn_server_info_file") == 0) {         writePortNumToServerInfoFile = true;         sprintf(serverInfoFileName, "%s", argv[tvar+1]);         argv[tvar++] = NULL;         argv[tvar]   = NULL;         continue;      }   }   port = getCommServicePort();   if (writePortNumToServerInfoFile) {      FILE* fh;      fh = fopen(serverInfoFileName, "w");      if (fh == NULL) {         result = "Unable to create target connectivity server info file\n";         return(result);      }      (void)fprintf(fh, "Server Port Number: %d\n", (unsigned int)port);      fclose(fh);   }}if (startParamTuningTgtAppSvc()) {    result = "Could not start ParamTuning app service";    return(result);}
    return(result);
   }
